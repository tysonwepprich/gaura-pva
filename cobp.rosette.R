...
sink(file="gaura.dm.rosette.txt")
cat("
...
# sigma.fl = runif(1, 0, 1))
pars.rosette <- c("lambda", "alpha", "r", "sigma.nu", "iota", "N", "mean.flower", "site.r", "site.beta", "lam", 
                 "sigma.beta", "beta.mean", "mean.r", "sigma.r", "sigma.fl", "creek", "total")

jm.rosette <- jags.model("gaura.dm.rosette.txt", dat.rosette, n.chains=3, n.adapt=50000)
update(jm.rosette, n.iter=50000)
jc.rosette <- coda.samples(jm.rosette, pars.rosette, n.iter=1000, thin = 1)
saveRDS(jc.rosette, "rosette_mcmc.rds")
...
MCMCplot(jc.rosette, params = 'beta.mean',ref_ovl = TRUE,
         labels = c(paste("Population growth:", c('Year', 'Precip (t-1)', 'Precip (t)', 
                                                 'Temp (t-1)', 'Temp (t)', 'N (t-1)'), sep = " "),
                    paste("Flowering rate:", c('Year', 'Precip (t-1)', 'Precip (t)', 
                                               'Temp (t-1)', 'Temp (t)', 'N (t-1)'), sep = " ")))
