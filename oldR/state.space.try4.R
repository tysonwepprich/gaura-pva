...
veg$total <- veg$Flow04 + veg$Veg04
veg$ratio <- veg$Flow04/veg$total

mod <- glm(cbind(Flow04, total - Flow04) ~ total, family = binomial(logit), data = veg)
summary(mod)
...
cat("
    ...
    R[t,j] <- r[t,j] + a0*rain0[t,1] + a1*rain1[t,1] + a2*rain2[t,1]
    + b1*X1[t,j] + b2*X2[t,j] + b3*X3[t,j]
    ...
    for (t in 1:nyear){
    for (j in 1:6){
    CC[t,j] <- exp(logN.est[t,j+7])
    }
    for (j in 1:5){
    DC[t,j] <- exp(logN.est[t,j+2])
    }
    for (j in 1:2){
    UC[t,j] <- exp(logN.est[t,j])
    }
    Crow[t] <- sum(CC[t,])
    Diamond[t] <- sum(DC[t,])
    Unnamed[t] <- sum(UC[t,])
...
sc.y <- scale(y)
year <- 1992:(2013 + pyears)

nyear  <- length(year)

# Bundle data
bugs.data <- list(y = log(y + 1), nyear = nyear,
                  rain0 = scale(rain$mm[c(4:28)]), 
                  rain1 = scale(rain$mm[c(3:27)]), 
                  rain2 = scale(rain$mm[c(2:26)]),
                  X1 = sc.y[c(3:24), ], 
                  X2 = sc.y[c(2:23), ],
                  X3 = sc.y[c(1:22), ])
...
inits <- function(){list(logN.est = rbind(rnorm(13, 5, 0.01),  matrix(data = rep(NA, (nyear-1)*13), ncol = 13)), 
                         ...
                         b1 = rnorm(1, 0, 1), b2 = rnorm(1, 0, 1), b3 = rnorm(1, 0, 1))}

# Parameters monitored
params <- c("mean.r", "sigma2.p", "sigma2.r", "a0", "a1", "a2", "b1", "b2", "b3", "Crow", "Diamond", "Unnamed", "logN.est") 
...
for (j in 1:13){
  ...
  points(y[c(4:25),j], type = "l", col = "black", lwd = 2)
  points(fitted[,j], type = "l", col = "blue", lwd = 2)
}

fitted <- lower <- upper <- numeric()
#draw figure of 3 subpopulations
for (i in 1:nyear){
  fitted[i] <- mean(out$BUGSoutput$sims.list$Diamond[,i])
  lower[i] <- quantile(out$BUGSoutput$sims.list$Diamond[,i], 0.025)
  upper[i] <- quantile(out$BUGSoutput$sims.list$Diamond[,i], 0.975)
}
...
axis(2, las = 1)
axis(1, at = 1:nyear, labels = year)
polygon(x = c(1:nyear, nyear:1), y = c(lower, upper[nyear:1]), col = "gray90", border = "gray90")
points(rowSums(y[4:25,3:7]), type = "l", col = "black", lwd = 2)
